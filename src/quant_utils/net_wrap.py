...
def wrap_modules_in_net(net, cfg):
    ...
    silu_module_types = {"act": "qact", "act_b": "qact", "act_z": "qact"}
    softplus_module_types = {"softplus": "qsoftplus", "softplus_b": "qsoftplus"}
    norm_module_types = {"norm": "qnorm", "norm_f": "qnorm"}
    ...
    for name,m in it:
        ...
        if isinstance(m,nn.Linear) & ("x_proj" in name):
            ...
            model = "tiny"
            if "x_proj_b" in name:
                r_D = np.loadtxt(f'./reparam_factors/{model}_factor/r_D/{layer_idx}_b.txt', dtype=np.float32)
            else:
                r_D = np.loadtxt(f'./reparam_factors/{model}_factor/r_D/{layer_idx}_f.txt', dtype=np.float32)
        ...
        if isinstance(m, nn.SiLU):
            idx = idx + 1 if idx != 0 else idx
            new_m = cfg.get_module(silu_module_types[name[idx:]])
            replace_m = new_m
            wrapped_modules[name] = new_m
            setattr(father_module, name[idx:], replace_m)
        if isinstance(m, nn.Softplus):
            idx = idx + 1 if idx != 0 else idx
            new_m = cfg.get_module(softplus_module_types[name[idx:]])
            replace_m = new_m
            wrapped_modules[name] = new_m
            setattr(father_module, name[idx:], replace_m)
        if isinstance(m, RMSNorm):
            idx = idx + 1 if idx != 0 else idx
            new_m = cfg.get_module(norm_module_types[name[idx:]], m.hidden_size, m.eps)
            new_m.weight.data = m.weight.data
            new_m.bias = m.bias
            replace_m = new_m
            wrapped_modules[name] = new_m
