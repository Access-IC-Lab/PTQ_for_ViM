...
class AsymmetricMinMaxConv1d(MinMaxQuantConv1d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class AsymmetricQuantileConv1d(QuantileQuantConv1d):
    def __init__(self,
        in_channels: int,
        out_channels: int,
        kernel_size,
        ...
        super().__init__(in_channels,out_channels,kernel_size,stride,padding,dilation,groups,bias,padding_mode,mode=mode,w_bit=w_bit,i_bit=i_bit,o_bit=o_bit,bias_bit=bias_bit,w_quantile=w_quantile,i_quantile=i_quantile,o_quantile=o_quantile)
...
class PTQSLQuantConv1d(MinMaxQuantConv1d):
    """
    PTQSL on Conv1d
    weight: (oc,ic,kl) -> (oc,ic*kl) -> divide into sub-matrixs and quantize
    input: (B,ic,L), keep this shape
    ...
    def __init__(self, in_channels: int,
        ...
        n_L=1, init_layerwise=False):
        super().__init__(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias, padding_mode=padding_mode, mode=mode, w_bit=w_bit, i_bit=i_bit, o_bit=o_bit, bias_bit=bias_bit)
        self.metric = metric
    ...
    def quant_weight_bias(self):
        # self.weight_interval shape: n_L, 1
        oc,ic,kl=self.weight.data.shape
        w_sim = self.weight.view(self.n_L, -1)
        w_sim = (w_sim/self.w_interval).round_().clamp(-self.w_qmax,self.w_qmax-1).mul_(self.w_interval)
        w_sim = w_sim.view(oc,ic,kl)
        return w_sim, self.bias
    ...
    def _search_best_w_interval(self, x, weight_interval_candidates):
        ...
        for p_st in range(0,self.eq_n,self.parallel_eq_n):
            ...
            cur_w_interval = weight_interval_candidates[p_st:p_ed] # shape: parallel_eq_n,1,1,1
            # quantize weight and bias
            oc,ic,kl = self.weight.data.shape
            w_sim = self.weight.unsqueeze(0) # shape: 1,oc,ic,kl
            w_sim = (w_sim/cur_w_interval).round_().clamp_(-self.w_qmax,self.w_qmax-1)*(cur_w_interval) # shape: parallel_eq_n,oc,ic,kl
            ...
            # quantize output
            out = F.conv1d(x_sim, w_sim, bias_sim, self.stride, self.padding, self.dilation, self.groups).flip(1) # shape: B,parallel_eq_n*oc,fl
            out = torch.cat(torch.chunk(out.unsqueeze(1), chunks=p_ed-p_st, dim=2), dim=1) # shape: B,parallel_eq_n,oc,fl
            out_sim = self.quant_output(out) # shape: B,parallel_eq_n,oc,fl
            # calculate similarity and store them
            similarity = self._get_similarity(self.raw_out, out_sim, self.metric, dim=2) # shape: B,parallel_eq_n,fl
            similarity = torch.mean(similarity, dim=[0,2]) # shape: parallel_eq_n
            similarities.append(similarity)
        # store best weight interval and store in tmp_w_interval
        similarities = torch.cat(similarities, dim=0)
        w_best_index = similarities.argmax(dim=0).view(1,1,1,1)
        self.w_interval = torch.gather(weight_interval_candidates,dim=0,index=w_best_index).squeeze()

    def _search_best_i_interval(self, x, input_interval_candidates):
        similarities = []
        for p_st in range(0,self.eq_n,self.parallel_eq_n):
            p_ed = min(self.eq_n, p_st+self.parallel_eq_n)
            cur_i_interval = input_interval_candidates[p_st:p_ed] # shape: parallel_eq_n,1,1,1
            # quantize weight and bias 
            w_sim, bias_sim = self.quant_weight_bias()
            # quantize input
            B,ic,il = x.shape
            x_sim=x.unsqueeze(0) # shape: 1,B,ic,il
            x_sim=(x_sim/(cur_i_interval)).round_().clamp_(-self.i_qmax,self.i_qmax-1)*(cur_i_interval) # shape: parallel_eq_n,B,ic,il
            ...
            out = torch.cat(torch.chunk(out.unsqueeze(0), chunks=p_ed-p_st, dim=1), dim=0) # shape: parallel_eq_n,B,oc,fl
            out_sim = self.quant_output(out) # shape: parallel_eq_n,B,oc,fl
            # calculate similarity and store them
            similarity = self._get_similarity(self.raw_out.transpose(0,1), out_sim, self.metric, dim=2) # shape: parallel_eq_n,B,fl
        ...
        i_best_index = similarities.argmax(dim=0).view(1,1,1,1) # shape: 1,1,1,1
    ...
    def _search_best_o_interval(self, x, output_interval_candidates):
        ...
        for p_st in range(0,self.eq_n,self.parallel_eq_n):
            p_ed = min(self.eq_n, p_st+self.parallel_eq_n)
            cur_o_interval = output_interval_candidates[p_st:p_ed]
            # quantize weight and bias 
            w_sim, bias_sim = self.quant_weight_bias()
            # quantize input
            x_sim = self.quant_input(x)
            ...
            out_sim=out.unsqueeze(0) # shape: 1,B,oc,fl
            out_sim=(out_sim/(cur_o_interval)).round_().clamp_(-self.o_qmax,self.o_qmax-1)*(cur_o_interval) # shape: parallel_eq_n,B,oc,fl
            # calculate similarity and store them
            similarity = self._get_similarity(self.raw_out.transpose(0,1), out_sim, self.metric, dim=2) # shape: parallel_eq_n,B,fl
            similarity = torch.mean(similarity, dim=[1,2]) # shape: parallel_eq_n
            similarities.append(similarity)
        ...
        similarities = torch.cat(similarities, dim=0) # shape: eq_n
        o_best_index = similarities.argmax(dim=0).view(1,1,1,1)
        self.o_interval = torch.gather(output_interval_candidates,dim=0,index=o_best_index).squeeze()

    def _initialize_intervals(self, x, out):
        self.w_interval=((self.weight.abs().max())/(self.w_qmax-0.5)).detach()
        self.i_interval=(x.abs().max()/(self.i_qmax-0.5)).detach()
        self.o_interval=(out.abs().max()/(self.o_qmax-0.5)).detach()
        # if self.init_layerwise:
        #     self.w_interval = ((self.weight.abs().max())/(self.w_qmax-0.5)).view(1,1).repeat(self.n_L,1) # shape: n_L,1
        # else:
        #     self.w_interval = (self.weight.view(self.n_L,-1).abs().amax([1],keepdim=True)/(self.w_qmax-0.5))
    ...
    def calibration_step2(self, x):
        ...
        # put raw outs on GPU
        self.raw_out = self.raw_out.to(x.device).unsqueeze(1)  # shape: B,1,oc,L

        # put raw grad on GPU
        self.raw_grad = self.raw_grad.to(x.device) if self.raw_grad != None else None

        # prepare weight intervals and similarities
        weight_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).cuda().view(-1,1,1,1) * self.w_interval # shape: eq_n,1,1,1
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).cuda().view(-1,1,1,1) * self.i_interval # shape: eq_n,1,1,1
        output_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).cuda().view(-1,1,1,1) * self.o_interval # shape: eq_n,1,1,1
...
class KScaleChannelWiseQuantConv1d(MinMaxQuantConv1d):
    def __init__(self,
        in_channels: int,
        out_channels: int,
        kernel_size,
    ...
    def calibration_step2(self, x):
        self.w_interval=(self.weight.data.abs().max()/(self.w_qmax-0.5)).detach()
        self.i_interval=(x.abs().max()/(self.i_qmax-0.5)).detach()
        out = F.conv1d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups) # shape: B,oc,fl
