...
#     def forward(self, h, deltaA, deltaB_x, C, step):
#         h = deltaB_x[:, :, step] + deltaA[:, :, step] * h # B, d_inner, d_state = B, 384, 16
#         y = torch.matmul(h, C[:, :, step].unsqueeze(-1)).squeeze(-1) # B, d_inner = B, 384
...
class QuantSSM(SequentialSSM):
    ...
    # def _kmeans(self, tensor, k, num_iters=100, dim=-1):
    #     """
    #     Example: 
